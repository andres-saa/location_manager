    ...
    <div class="flex flex-col gap-3 mb-6 md:flex-row md:items-end">
      <div class="flex-1 min-w-0">
        ...
        <div class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100 text-gray-700 text-base flex items-center gap-2">
          <img 
            :src="getCountryFlagUrl(countryStore.getCountryOption?.value || 'colombia')" 
            :alt="countryStore.getCountryOption?.label || 'País'"
            class="w-5 h-5 object-contain flex-shrink-0"
          />
          <span>{{ countryStore.getCountryOption?.label || 'País' }}</span>
        </div>
      </div>
      <div class="flex-1 min-w-0">
...
const country = computed(() => {
  ...
  return countryOption?.label || 'País'
})
...
const getPolygonForMap = () => {
  ...
  if (result.value && result.value.matching_polygons.length > 0 && result.value.matching_polygons[0]) {
